    ...
    <section className="rulesSection">
      <article className="panel">
        ...
        <div className="rules">{rules.map((rule, index) => <div className="ruleCard" key={rule.id}>
          ...
          <label>Texte ou motif a detecter<input value={rule.pattern} onChange={(event) => updateRule(rule.id, { pattern: event.target.value })} placeholder={rule.isRegex ? "sk_live_[A-Za-z0-9]+" : "Texte exact, ex : client-42"} /></label>
          <div className="ruleMeta"><label>Categorie<select value={rule.category} onChange={(event) => updateRule(rule.id, { category: event.target.value as SensitiveCategory })}>{categories.map((category) => <option key={category} value={category}>{categoryLabels[category]}</option>)}</select></label><label className="toggle" title="Coche uniquement si ton motif est une expression reguliere (regex), pour attraper plusieurs variantes d'un coup. Exemple : sk_live_[A-Za-z0-9]+ trouve toutes les cles qui commencent par sk_live_. Si tu decoches, le texte est cherche exactement tel que tu l'ecris."><input type="checkbox" checked={rule.isRegex} onChange={(event) => updateRule(rule.id, { isRegex: event.target.checked })} />Regex<span className="infoDot" aria-hidden="true">i</span></label></div>
          <p className="ruleHint">{rule.isRegex ? "Mode regex (avance) : le motif est interprete comme une expression reguliere pour attraper des variantes." : "Mode texte : SanizOtter cherche ce texte exactement, tel quel. Coche Regex pour les motifs variables."}</p>
        </div>)}</div>
      </article>
    </section>
